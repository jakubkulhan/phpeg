...
function {$this->phpeg->getName()}(\$s, array \$opts = array()) {\n
    ...
    if (!class_exists('__{$this->phpeg->getName()}')) {
        ...
        class __{$this->phpeg->getName()} {
            ...
            public function __construct(\$_s, array \$opts = array())
            {
            ...
            public function _p()
            {
                ...
                \$this->_p = func_get_arg(0);

                \$this->_maxp = max(\$this->_maxp, \$this->_p);
                if (func_num_args() === 2 && func_get_arg(1)) {
                    \$this->_maxp = \$this->_p;
                }
            }

            public function _getmemo(\$r, \$p)
            {
                \$k = ((string) \$r) . ':' . ((string) \$p);
            ...
            public function _setmemo(\$r, \$p, \$v)
            {
                \$this->_memo[((string) \$r) . ':' . ((string) \$p)] = \$v;
            }

            public function _maxp()
            {
                return \$this->_maxp;
            }
    ...
    \$ret = \$i->{$this->phpeg->getStartingRuleName()}();
    return array_merge(\$ret, array(\$i->_maxp()));
}
...
E;
    ...
    private function gmr0($i, $tree)
    {
        return "\$_$i = array(TRUE, array());\n" .
               "do {\n" .
                   "\$_pos$i = \$this->_p();\n" .
                   $this->g($i + 1, $tree) .
                   "if (!\$_" . ($i + 1) . "[0]) { \$this->_p(\$_pos$i, TRUE); }\n" .
                   "else { \$_{$i}[1][] = \$_" . ($i + 1) . "[1]; }\n" .
               "} while (\$_" . ($i + 1) . "[0]);\n"//;
               .
    ...
    private function glit($i, $s)
    {
        ...
        foreach (str_split($s) as $c) {
            if ($c === "\\") { $encapsed_s .= "\\\\"; }
            else if (ctype_print($c) || $c === ' ') { $encapsed_s .= $c; }
            else {
                $hex = dechex(ord($c));
                $encapsed_s .= '\x' . (strlen($hex) < 2 ? str_pad($hex, 2, '0', STR_PAD_LEFT) : $hex);
            }
        }
    ...
    private function gcode($code)
    {
        ...
        $this->methods[] = "private function " . $methodname . "(" . implode (", ", $params) . ") {\n" .
                           $code .
                           "\n}\n";
